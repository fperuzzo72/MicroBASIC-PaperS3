...
void refreshFileList() {
  ...
  root.close();
  // The X4's SDCardManager had a sleep() to drop the card's power draw
  // between operations; this SDK's does not expose one.
...
void loadFile(const char* filename) {
  ...
  // The X4 set its global UIState here. This port has no such global: the
  // browser module owns the transition into the editor screen itself, so
  // loading a file is only a data operation.
